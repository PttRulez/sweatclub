...
routes.post('/', bearerGuard, validateSchema(PlaceSchema), async (req: Request<{}, {}, CreatePlaceDto>, res, _) => {

	const place = await placeService.createNewPlace({...req.body, creatorId: req.user.id});
